...
class SumAndCurrencyVC: UIViewController {
    ...
    func setBorder(to views: [UIView]) {
    ...
    @IBAction func textChanged(_ sender: UITextField) {
        ...
        if let sum = Int(tfSum.text!) {
            ...
            //cb
            var cashBack = sumInRubbles/100
            cashBack *= 100
            cashBack.round(.toNearestOrEven)
            cashBack /= 100
            let cashBackString = "+\(cashBack) ₽"
            
            labTotalSum.text = sumInRubblesString
            labCashBack.text = cashBackString
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "toRecipientVC" {
            
            let nextVC = segue.destination as! RecipientVC
            
            nextVC.currentRegion = currentRegion
        }
    }
