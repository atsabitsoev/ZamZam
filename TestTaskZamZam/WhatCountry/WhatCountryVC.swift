...
class WhatCountryVC: UIViewController {
    ...
    func setColoredPlaceHolder() {
        ...
        myPlaceholder = NSMutableAttributedString(string: text, attributes: [.font : UIFont(name: "Noto Sans",
                                                                                            size: 16)!])
        myPlaceholder.addAttribute(NSAttributedString.Key.foregroundColor, value: UIColor(red: 86/255, green: 192/255, blue: 253/255, alpha: 0.6), range: NSRange(location: 0, length: myPlaceholder.length))
        myPlaceholder.addAttribute(NSAttributedString.Key.kern, value: -0.5, range: NSRange(location: 0, length: myPlaceholder.length))
        ...
        let v = UIView(frame: CGRect(x: 0, y: 0, width: 30, height: 32))
        
        v.addSubview(imageView)
        imageView.frame = CGRect(x: 9, y: 7, width: 18, height: 18)
        textFieldSearch.leftView = v
        textFieldSearch.leftView?.alpha = 0.6
        
        
        print(textFieldSearch.leftView?.bounds)
    }
